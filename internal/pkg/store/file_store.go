...
func (s *FileStore) LoadAll() ([]*domain.ColdStoreEntry, error) {
	...
	entries := []*domain.ColdStoreEntry{}
	scanner := bufio.NewScanner(file)
	for scanner.Scan() {
		data := scanner.Bytes()
		entry := &domain.ColdStoreEntry{}
...
func (s *FileStore) Save(entry *domain.ColdStoreEntry) error {
	s.mutex.Lock()
